...
class Curiosity(Exploration):
    ...
    def __init__(self,
        ...
        self._teacher_net = ModelCatalog.get_model_v2(
            self.model.obs_space,
            self.action_space,
            self.actions_space, # my addition - might be the wrong format
    ...
    def _postprocess_torch(self, policy, sample_batch):
        ...
        pred_teacher_actions = self.model._teacher_net({
            SampleBatch.OBS:
                torch.from_numpy(sample_batch[SampleBatch.OBS])
        })
        # Want to compare pred_teacher_actions with the result of policy
        # to get reward. Also, create a loss when compared with
