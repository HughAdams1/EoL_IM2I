...
class mod_TorchCentralizedCriticModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        self.central_vf = nn.Sequential(
            SlimFC(input_size, 184, activation_fn=nn.Tanh),
            SlimFC(184, 1),
        )
    ...
    def central_value_function(self, obs, opponent_obs, opponent_actions):
        #print(obs.shape)
        #print(opponent_obs.shape)
        #print(opponent_actions.type)
        #print(opponent_actions)
        #print(torch.nn.functional.one_hot(opponent_actions.long()))
        input_ = torch.cat([
            obs, opponent_obs,
...
if __name__ == "__main__":
    ...
    config["multiagent"] = {
        "env": "spread",
        "policies": {"ppo_policy": (None, observation_space, action_space, {
    ...
    config_alt = {
        ...
        "multiagent": {
            "policies": {
                "PPO": (None, observation_space, action_space, {
                    "framework": args.framework,
                }),
                "A3C": (None, observation_space, action_space, {
                    "framework": args.framework,
                }),
            },
            "policy_mapping_fn": (
                lambda agent_id, episode, **kwargs: "PPO"),
        },
        "model": {
