...
MAX_STEPS = 50
total_steps = 0
training_score_history = {agent:[] for agent in env.agents}
evaluate = False
best_score = 0
...
for i in range(TRAINING_GAMES):
    ...
    while not any(done_list):
        ...
        if episode_step > MAX_STEPS:
            done_list = [True for agent in env.agents]
    ...
    for agent in env.agents:
        training_score_history[agent].append(scores[agent])
        ...
        #avg_scores = {agent:np.mean(score_history[agent][-100:]) for agent in env.agents}
        #avg_score = np.mean(score_history[-100:])
        #print('episode', i, 'average scores:',avg_scores)
#env.close()
#print(score)
...
score_history = {agent:[] for agent in env.agents}
